...
#define is_dput_memory_op(flags) ((DETERMINE_ZERO_FILL | DETERMINE_VM_COPY) & \
		flags)
#define is_dget_memory_op(flags) ((DETERMINE_MERGE | DETERMINE_ZERO_FILL | \
		...
		size_t, size, unsigned long, dststart)
{
	...
	struct list_head *list;
	int ret;
	int is_mem_op = is_dget_memory_op(flags);
    int lowbits_clear;
	//DEFINE_WAIT(wait);
	...
	PRINT_OFTEN(printk("Entering DGET (%d) %08lx %08lx %08lx %08lx %08lx\n",
				current->pid,child_dpid,flags,start,size,dststart);)
	/* Disallow certain flags, sanity check arguments: */
    lowbits_clear = (0 == ((PAGE_SIZE-1) & PAGE_ALIGN(dststart - start)));
    if (is_mem_op && (start + size <= start || !lowbits_clear))
    {
        return -EINVAL;
    }
    else if (DETERMINE_REGS & flags)
	{
        /* Can't copy registers in dget. */
		return -EINVAL;
	}
    ...
    {
        /* Can't merge and copy memory at the same time. */
        return -EINVAL;
    }
    else if ((DETERMINE_CHILD_STATUS & flags) && is_mem_op)
	{
        /* Can't fetch status and be a memory op at the same time. */
		return -EINVAL;
	}
